...
mp.events.add('seatsactive',() => {
	mp.game.controls.disableControlAction(2, 24, true);
	 ...
	 mp.players.local.canPunch = false;
	 canPunch = false;
...
mp.events.add('playerEnterColshape', (shape) => {
	...
	if(shape.casinoTable !== undefined && lpCasinoTable == null && interactingWithTable == null)
	{
		...
		mp.game.graphics.notify(`~g~[E]~s~ Сесть за стол~n~Ставка: ~b~${tablesBets[casinoTableToJoin][0]}~s~ - ~b~${tablesBets[casinoTableToJoin][1]}~s~ долларов`);
		mp.game.controls.disableControlAction(2, 24, true);
	 mp.game.controls.disableAllControlActions(2);
	 mp.players.local.canPunch = false;
	 canPunch = false;
